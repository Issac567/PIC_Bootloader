...
* [MPLAB Ecosystem – Microchip (Recommend MPLAB X 6.05 with Both Pickit 3/MBLAP Snap)](https://www.microchip.com/en-us/tools-resources/archives/mplab-ecosystem)
...
5. Upload/flash **(Chip with download icon)**  the bootloader and application with MBLAB.  Which allows bootloader to flash, erase and verify in B4J.  We need the bootloader minimal.  If set as loadables, it will flash both bootloader and application.
...
4. Make sure minimal bootloader is flashed before B4J usage.  **Chip with download icon** to flash.
...
bootloader.c and application.c share the same function name calls across all 7 PIC projects. Adding support for additional PIC devices requires only minor modifications to the C codes, config.h and .map configuration.
