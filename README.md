...
* **`ESP32_BootloaderUploader/`**: Contains the **ESP32 uploader** utility used to flash compiled `.bin` files to the target hardware via a PC. flash.bin and config.map required files must be in SD Card and B4J currently can export those files for ESP 32. Only BLE is supported at 20 MTU only.
